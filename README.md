...
经常要访问国内外的大模型平台，各家 API 的调用费用计价方式五花八门——有的用美元（$0.002/1K tokens），有的用人民币（¥0.01/1K tokens），还有的打包成各种积分或套餐。想货比三家的时候，得不停地在心里做汇率换算，非常麻烦。

于是做了这个扩展：**让网页上的金额自动按汇率转换**，省去手动计算的成本，一眼就能看出哪家更划算。

当然，它也不局限于大模型平台。浏览海外购物网站、查看国际新闻、阅读外币标价的内容时，都能用得上。
